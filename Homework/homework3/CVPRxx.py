...
def scrape_cvpr_from_dblp(start_year=2020):
    """
    从DBLP爬取CVPR会议论文信息(2020年至今)
...
def save_to_excel(df, filename):
    ...
    try:
        ...
        with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
            df_from_csv.to_excel(writer, index=False, sheet_name='CVPR Papers')
...
if __name__ == "__main__":
    try:
        # 获取数据
        print("正在从DBLP获取CVPR会议论文...")
        cvpr_papers_df = scrape_cvpr_from_dblp()
        
        # 显示前几行数据
        if not cvpr_papers_df.empty:
            print("\n获取到的论文样例:")
            print(cvpr_papers_df.head())
            
            # 保存到Excel
            save_to_excel(cvpr_papers_df, "cvpr_papers_2020_present.xlsx")
        else:
