...
def scrape_ijcai_from_dblp(start_year=2020):
    """
    从DBLP爬取IJCAI会议论文信息(2020年至今)
    ...
    """
    base_url = "https://dblp.org/db/conf/ijcai/"
    papers = []
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
    }
    
    print("开始从DBLP爬取IJCAI会议论文信息...")
    
    for year in tqdm(range(start_year, 2026), desc="处理年份"):
        url = f"{base_url}ijcai{year}.html"
        try:
            # 添加延迟避免被封
            time.sleep(1)
...
def save_to_excel(df, filename):
    ...
    try:
        ...
        with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
            df_from_csv.to_excel(writer, index=False, sheet_name='IJCAI Papers')
...
if __name__ == "__main__":
    try:
        # 获取数据
        print("正在从DBLP获取IJCAI会议论文...")
        ijcai_papers_df = scrape_ijcai_from_dblp()
        
        # 显示前几行数据
        if not ijcai_papers_df.empty:
            print("\n获取到的论文样例:")
            print(ijcai_papers_df.head())
            
            # 保存到Excel
            save_to_excel(ijcai_papers_df, "ijcai_papers_2020_present.xlsx")
        else:
