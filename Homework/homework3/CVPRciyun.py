...
# 读取数据（修改为CVPR的Excel文件路径）
data = pd.read_excel(r"C:\python\pachong\zuoye\output\cvpr_papers_2020_present.xlsx")  # 确保文件名正确
...
def get_word_freq(keywords):
    ...
    # 过滤掉一些通用词（可根据CVPR论文调整）
    common_words = {'learning', 'network', 'model', 'based', 'using', 'deep', 'neural', 'detection', 'recognition', 'prediction', 'image', 'vision', 'computer'}
...
generate_wordcloud(freq_2020, 'CVPR 2020 Research Hotspots')
generate_wordcloud(freq_2021, 'CVPR 2021 Research Hotspots')
generate_wordcloud(freq_2022, 'CVPR 2022 Research Hotspots')
generate_wordcloud(freq_2023, 'CVPR 2023 Research Hotspots')
generate_wordcloud(freq_2024, 'CVPR 2024 Research Hotspots')